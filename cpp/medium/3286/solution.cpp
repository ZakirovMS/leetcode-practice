                                                                  ...
                                                                  std::vector< std::vector< int > > & weightMtx)
{
  ...
  for (auto i = wave.begin(); i != wave.end(); ++i)
  {
    int m = i->first;
    int n = i->second;
    if (m != 0 && (weightMtx[m - 1][n] == -1 || weightMtx[m - 1][n] > weightMtx[m][n] + grid[m - 1][n]))
    {
      newWave.push_back(std::make_pair(m - 1, n));
      weightMtx[m - 1][n] = weightMtx[m][n] + grid[m - 1][n];
    }
    if (n != 0 && (weightMtx[m][n - 1] == -1 || weightMtx[m][n - 1] > weightMtx[m][n] + grid[m][n - 1]))
    {
      newWave.push_back(std::make_pair(m, n - 1));
      weightMtx[m][n - 1] = weightMtx[m][n] + grid[m][n - 1];
    }
    if (m != grid.size() - 1 && (weightMtx[m + 1][n] == -1 || weightMtx[m + 1][n] > weightMtx[m][n] + grid[m + 1][n]))
    {
      newWave.push_back(std::make_pair(m + 1, n));
      weightMtx[m + 1][n] = weightMtx[m][n] + grid[m + 1][n];
    }
    if (n != grid[0].size() - 1 && (weightMtx[m][n + 1] == -1 || weightMtx[m][n + 1] > weightMtx[m][n] + grid[m][n + 1]))
    {
      newWave.push_back(std::make_pair(m, n + 1));
      weightMtx[m][n + 1] = weightMtx[m][n] + grid[m][n + 1];
    }
